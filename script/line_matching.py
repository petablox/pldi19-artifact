...
# ./line_matching.py [path to version n] [path to version n+1]
...
def get_filepaths(directory):
    ...
    for root, directories, files in os.walk(directory):
        for filename in [ f for f in files if f.endswith('.c') or f.endswith('.h') ]:
...
def line_match(file1, file2):
    ...
    num_deleted_lines = 0
    num_changed_lines = 0
    for line in [ line for line in result
    ...
    if len(added_lines) == 0 and info == []:
        return (0, 0, 0, [])

    num_lines = sum(1 for line in codecs.open(file2, 'r', encoding='utf-8', errors='ignore'))
    matching = []
    diff = 0
...
for f in [f for f in added_files if 'test' not in f and 'gnulib' not in f]:
    f = new_dir + '/' + f
    num_lines = sum(1 for line in codecs.open(f, 'r', encoding='utf-8', errors='ignore'))
...
for f in common_files:
    old_file = old_dir + '/' + f
    new_file = new_dir + '/' + f
    num_lines = sum(1 for line in codecs.open(new_file, 'r', encoding='utf-8', errors='ignore'))
    total_lines += num_lines
    if not filecmp.cmp(old_file, new_file, shallow=False):
        (added_lines, deleted_lines, changed_lines, matching) = line_match(old_file, new_file)
        ...
        print("  {}: {} total, {} added, {} deleted, {} changed, {} unchanged".format(f, num_lines, added_lines, deleted_lines, changed_lines, unchanged_lines))
        if matching != []:
...
print("Total # changed lines: {}".format(total_added_lines + total_changed_lines))
